...
const KnowledgeGraph: React.FC = () => {
  ...
  const fetchData = async (query: string) => {
    ...
    } catch (err) { setError("数据链路异常，请确认网络环境或 API 状态"); } finally { setLoading(false); }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-10 p-4 bg-slate-50 min-h-screen font-sans">
      ...
      <div ref={containerRef} className="bg-[#0f172a] rounded-2xl shadow-2xl border border-slate-800 relative overflow-hidden" style={{ height: '1000px' }}>
        {loading ? (
        ...
        <div className="absolute top-6 right-6 bg-slate-100/90 backdrop-blur-md p-4 rounded-xl shadow-2xl border border-slate-200 z-10 w-36 pointer-events-none">
            <p className="font-extrabold text-slate-800 mb-3 text-xs border-b border-slate-300 pb-1 text-center">图谱示例</p>
